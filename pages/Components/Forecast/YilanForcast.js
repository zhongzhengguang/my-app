...
const YilanForecast = () => {
  ...
  useEffect(() => {
    axios
      .get(url)
      .then((response) => {
        setData(response.data.cwbopendata.dataset.location[2]);
        setWeatherElementTime(
          response.data.cwbopendata.dataset.location[2].weatherElement[3]
            .time[0]
        );
        setWaveHieght(
          response.data.cwbopendata.dataset.location[2].weatherElement[3]
        ...
        setWaveType(
          response.data.cwbopendata.dataset.location[2].weatherElement[4]
            .time[0].parameter
        );
        setWindDir(
          response.data.cwbopendata.dataset.location[2].weatherElement[1]
            .time[0].parameter
        );
        setwindSpeed(
          response.data.cwbopendata.dataset.location[2].weatherElement[2]
            .time[0].parameter
        );
      })
      .catch((error) => {
